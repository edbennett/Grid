...
int main (int argc, char ** argv)
{
  ...
  long unsigned int single_site_flops = 8*(Nc*Nc-1)*(7+16*(Nc*Nc-1));
  ...
  std::cout<<GridLogMessage << "Benchmarking Wilson operator in the adjoint representation" << std::endl;
  ...
  typename WilsonAdjFermionR::ImplParams params;
  RealD mass=0.1;

  AdjointRepresentation::LatticeField Umu(&Grid); random(pRNG,Umu);
  WilsonAdjFermionR Dw(Umu,Grid,RBGrid,mass,params);

  LatticeAdjFermion src   (&Grid); random(pRNG,src);
  LatticeAdjFermion result(&Grid); result=Zero();
  LatticeAdjFermion    ref(&Grid);    ref=Zero();
  LatticeAdjFermion    tmp(&Grid);    tmp=Zero();
  LatticeAdjFermion    err(&Grid);    tmp=Zero();

  std::vector<LatticeAdjColourMatrix> U(4,&Grid);
